...
void RBOTEvaluator::EvaluateRunConfiguration(
  ...
  SetUpTracker(run_configuration, renderer_geometry_ptr, tracker_ptr); //初始化所有的东西
  ResetBody(tracker_ptr, 0);  //加载物体，初始化颜色直方图
...
void RBOTEvaluator::ExecuteMeasuredTrackingCycle(
  ...
  tracker_ptr->CalculateBeforeCameraUpdate(); // 调用rigion_modality的函数进行颜色直方图的更新
  execution_times->calculate_before_camera_update = ElapsedTime(begin_time);
  tracker_ptr->UpdateCameras();  // 读取下一帧图片
  ...
  for (int corr_iteration = 0;
       corr_iteration < tracker_ptr->n_corr_iterations(); ++corr_iteration) {  // 迭代7次，scale==5,2,2,1,1,1,1
    ...
    begin_time = std::chrono::high_resolution_clock::now();
    tracker_ptr->CalculateCorrespondences(corr_iteration);  // 计算p(d|w,l)
    ...
    for (int update_iteration = 0;
         ...
         ++update_iteration) {  //循环两次，第一次全局优化，第二次局部优化，tracker_ptr->n_update_iterations() == 2
