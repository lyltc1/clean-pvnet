...
bool RegionModality::StartModality() {
  ...
  PrecalculatePoseVariables();  // 计算body2camera_pose_
...
bool RegionModality::CalculateCorrespondences(int corr_iteration) {
  if (!IsSetup()) return false;  // 确定RegionModality被初始化好

  PrecalculatePoseVariables();  // 计算body2camera_pose_和body2camera_rotation_和body2camera_rotation_xy_
  PrecalculateScaleDependentVariables(corr_iteration);  // 根据scale计算correspondence line的长度
  ...
  model_ptr_->GetClosestTemplateView(body2camera_pose_, &template_view); //在模板中找到与初始值最接近的模板

  // Iterate over n_lines
  std::vector<float> segment_probabilities_f(line_length_in_segments_);  //line_length_in_segments_==19
  ...
  for (auto data_point = begin(template_view->data_points);
       ...
       ++data_point) {
    DataLine data_line;
    CalculateBasicLineData(*data_point, &data_line); //计算center和n在图像上的表示，以及scaled 的 line的长度
    if (!IsLineValid(data_line.center_u, data_line.center_v,  // 确定这个line是有效的，再进行后续操作
                     data_line.continuous_distance))
      continue;
    //计算correspondence line上面每一个点属于前景背景的概率存储在segment_probabilities_f和segment_probabilities_b里面
    //计算n/s存储在 &data_line.normal_component_to_scale，计算delta_r存储在data_line.delta_r
    ...
    CalculateDistribution(segment_probabilities_f, segment_probabilities_b,
                          &data_line.distribution);  //计算p(d|w,l)，d∈[-5.5,-4.5,...,4.5,5.5]

    CalculateDistributionMoments(data_line.distribution, &data_line.mean,
                                 &data_line.standard_deviation,
                                 &data_line.variance);  //计算distribution的均值方差标准差
    data_lines_.push_back(std::move(data_line)); // std::move销毁前转移所有权，避免内存管理
  }
  return true;
}
...
bool RegionModality::CalculatePoseUpdate(int corr_iteration,
  ...
  for (auto &data_line : data_lines_) {
    ...
    if (update_iteration < n_newton_iterations_) { // n_newton_iterations_==1
    // 全局优化 看成是高斯分布
...
void RegionModality::PrecalculatePoseVariables() {
  // 计算body2camera_pose_和body2camera_rotation_和body2camera_rotation_xy_
  body2camera_pose_ =
...
void RegionModality::PrecalculateScaleDependentVariables(int corr_iteration) {
  // 根据line_length_in_segments_ == 19与sclae计算line_length_的长度以及相关变量
  if (corr_iteration < int(scales_.size()))  // 一共迭代7次，scale_分别为5，2，2，1，1，1，1
    scale_ = scales_[corr_iteration];
  else
    scale_ = 1;
  fscale_ = float(scale_);
  line_length_ = line_length_in_segments_ * scale_;  // line_length_in_segments_ == 19
...
void RegionModality::CalculateBasicLineData(const Model::PointData &data_point,
  ...
  // 计算data_point对应的data_line的信息，主要是图像中center投影normal投影和line的distance不出现前景背景误解
...
/* 确定correspondence line的长度大于最小值，中心落在图像上以及中心不被一致物体遮挡 */
  // Check if continuous distance is long enough
  if (continuous_distance < min_continuous_distance_) return false; //不能小于6
...
void RegionModality::CalculateDistribution(
  ...
  // 求p(d|w,l)=h_f(r-d)pf(r)+h_b(r-d)pb(r) d∈[-5.5,-4.5,...,4.5,5.5]
  // pf和pb有[-9,-8,...,8,9]的值
  // h_f和h_b有[-3.5,-2.5,...,2.5,3.5]的值
  ...
  distribution->resize(distribution_length_);  // 12
